...
async function RapierPhysics(THREE)
{
	...
	function hideMesh(meshName = '', meshInstanceIndex = 0)
	{
					...
					for(let j = 0; j < bodies.length; j++) 
					{
						...
						if(j == meshInstanceIndex)
						{
							...
							//(
								//function() 
								//{
									body.setTranslation(position.x, 2, position.z);
	...
	function setMeshTranslation(params) 
	{
		params = typeof params !== 'undefined'? params : {mesh: null, index: 0, translation: {x: 0, y: 0, z: 0}}; 
		
		if(mesh.isInstancedMesh) 
		{
			const 
			
			bodies	= meshMap.get(params.mesh),
			body	= bodies[params.index];
			body.setTranslation(params.translation);
		} 
		else if(mesh.isMesh) 
		{
			const 
			
			body = meshMap.get(params.mesh);
			body.setTranslation(params.translation);
		}
	}
	
	
	function setMeshLinearVelocity(mesh, linearVelocity = 2, index = 0, angleVelocity = {x: 3.0, y: 0.0, z: 1.0})
	{
		angleVelocity = typeof angleVelocity !== 'undefined'? angleVelocity : false; 
		...
		if(mesh.isInstancedMesh) 
		{
			...
			body = bodies[index]; if(angleVelocity != false) 
			body.setAngvel(angleVelocity);
		...
		else if(mesh.isMesh) 
		{
			...
			body = meshMap.get(mesh); if(angleVelocity != false) 
			body.setAngvel(angleVelocity);
			body.setLinvel(linearVelocity);
			body.setTranslation(position);
		}
	...
	function setMeshResetPhysics(meshPhysics)
	{
		...
		if(mesh.isInstancedMesh) 
		{
			...
			compose(position, rotation, array, index * 16);
		} 
		else if(mesh.isMesh) 
		{
			const
			...
			body.setTranslation	(position);	//mesh.position.copy	(body.translation());
			body.setRotation	(rotation);	//mesh.quaternion.copy	(body.rotation());
			body.setLinvel		(linearVelocity);
			body.setAngvel		(angularVelocity);
		}
	}
	...
	function setMeshReset(meshPhysics)
	{
		let
		
		mesh			= meshPhysics.mesh,
		meshIndex		= meshPhysics.meshIndex,
		mass			= meshPhysics.mass,
		position		= meshPhysics.position,
		rotation		= meshPhysics.rotation,
		linearVelocity	= meshPhysics.linearVelocity,
		...
		if(mesh.isInstancedMesh) 
		{
			...
			compose(position, rotation, array, meshIndex * 16);
		} 
		else if(mesh.isMesh) 
		{
			const
	...
	return {addMesh: addMesh, setMeshReset: setMeshReset, setMeshTranslation: setMeshTranslation, setMeshPosition: setMeshPosition, setMeshLinearVelocity: setMeshLinearVelocity, setMeshResetPhysics: setMeshResetPhysics, hideMesh: hideMesh};
